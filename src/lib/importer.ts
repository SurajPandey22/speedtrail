...
export interface Anomaly {
  ...
  autoResolved: boolean;     // true if we can fix it without user input
  resolvedValue?: string;    // what we'll use if approved
}
...
export interface ParsedExpense {
  ...
  description: string;
  paidByName: string;
  amount: number;                 // in INR (after conversion)
  originalAmount: number;
  originalCurrency: string;
  exchangeRate: number;
  splitType: 'equal' | 'unequal' | 'percentage' | 'share';
  splitWith: string[];            // normalized member names
  splitDetails: Record<string, number>;  // name → amount/percent/share
  isRefund: boolean;
...
const MEMBERSHIP_RANGES: Record<string, { joined: string; left?: string }> = {
...
export function detectAnomalies(
  ...
  for (const row of rows) {
    ...
    let splitType = row.split_type as ParsedExpense['splitType'];
...
function detectDuplicates(parsed: ParsedExpense[]) {
...
function fuzzyMatchMember(name: string): string | null {
  for (const known of KNOWN_MEMBERS) {
    if (levenshtein(name.toLowerCase(), known.toLowerCase()) <= 2) return known;
    if (known.toLowerCase().startsWith(name.toLowerCase().slice(0, 4))) return known;
  }
  return null;
}
...
export function computeSplits(expense: ParsedExpense): Record<string, number> {
